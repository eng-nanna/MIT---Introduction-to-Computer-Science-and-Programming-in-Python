...
def getWordScore(word, n):
    ...
    points = 0
    for chr in word:
        points += SCRABBLE_LETTER_VALUES[chr]
    if len(word) == n:
        points = (points*len(word)) + 50
    else:
        points *= len(word)
    return points
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        ...
        for chr in word:
            if chr in hand.keys() and freq[chr] <= hand[chr]:
                continue
            else:
                return False
        return True
    ...
    while(calculateHandlen(hand)>0):
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                total += score
                print("\"",user_word,"\" earned",score," points. Total:",total," points \n")
                # Update the hand 
